...
class DailyWeatherTableViewCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        self.backgroundColor = UIColor(hex: "EE7854")
        setupUI()
        // Initialization code
    ...
    func setupUI() {
        ...
        icon = UIImageView(frame: CGRect(x: dayLabel.frame.maxX, y: contentView.frame.minY, width: contentView.frame.width * 0.1, height: contentView.frame.height))
        icon.contentMode = .scaleAspectFit
        
        highTemp = UILabel(frame: CGRect(x: icon.frame.maxX + 20, y: contentView.frame.minY + 10, width: contentView.frame.width * 0.15, height: contentView.frame.height))
